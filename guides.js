...
const HELP = {
  ...
  tamper: ['What it does: Combines four independent forensic signals \u2014 ELA variance, noise inconsistency, edge discontinuity and block color statistics \u2014 into a composite tamper-confidence map.', 'The view shows TWO panels side by side: LEFT = the ORIGINAL picture with the edited/tampered areas tinted red and outlined, so you can still see the real content (the elements) behind the highlighted regions. RIGHT = the confidence heatmap (blue = likely safe, yellow = suspicious, red = high tamper likelihood).', 'Numbered boxes on the original mark each suspicious region; the legend lists totals (flagged % and region count).', 'The sensitivity setting controls the detection threshold: Low flags only strong anomalies, High flags anything slightly unusual. Smaller block sizes give finer detail but more noise.', 'Limits: Uniform re-compression (social media) flattens ELA; naturally textured areas can trigger false positives. Always corroborate with the ELA and Noise tools.']
};
